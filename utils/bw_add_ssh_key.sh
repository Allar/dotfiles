...
bw get template item | jq ".name=\"$BW_SSH_NAME\"" | jq ".login=$(bw get template item.login)" | jq ".login.username=\"$(cat ~/.ssh/id_rsa.pub)\"" | jq ".login.password=\"$(sed -e ':a' -e 'N' -e '$!ba' -e 's/\n/\\\\n/g' ~/.ssh/id_rsa)\"" | bw encode | bw create item
